...
**Next Public Release:** v1.2 (In Development)
...
### [ACTIVE] set-performance.sh (v3.2)
...
### [COMING SOON - v1.1] REST API and XO Token Integration
Foundational plumbing for all XO REST API calls, token authentication, and NFS
integration mechanics. Required dependency for v1.2 management features.

### [COMING SOON - v1.2] sync-vm-metadata.sh
...
### [COMING SOON - v1.2] set-permissions.sh
...
v1.2 (Planned - Next Public Release)
- sync-vm-metadata.sh: Bulk VM tagging via editable CSV (export / dry-run / sync modes)
- set-permissions.sh: Automated XO Resource Set and user permission management via REST API
- CSV as Source of Truth workflow for bulk VM metadata management
- Full XO Resource Set integration using resourceSet.* commands

v1.1 (Internal Milestone -- Not Released)
- REST API integration: XO token authentication and API call framework
- NFS integration mechanics: Centralized log aggregation across multiple pools
- Foundational plumbing for v1.2 management features
- custom.conf: Consolidated all override options (commented by default)
